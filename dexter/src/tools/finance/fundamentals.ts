...
export const getIncomeStatements = new DynamicStructuredTool({
  ...
  func: async (input) => {
    ...
    const { data, url } = await stocklens.getFinancialStatements(ticker);
    const statements = input.period === 'quarterly' ? data?.quarterly?.incomeStatement : data?.incomeStatement;
    let res = statements || [];
    if (input.limit) res = res.slice(0, input.limit);
    return formatToolResult(
      stripFieldsDeep(res, REDUNDANT_FINANCIAL_FIELDS),
      [url]
    );
  },
});
...
export const getBalanceSheets = new DynamicStructuredTool({
  ...
  func: async (input) => {
    const ticker = input.ticker.trim().toUpperCase();
    const { data, url } = await stocklens.getFinancialStatements(ticker);
    const statements = input.period === 'quarterly' ? data?.quarterly?.balanceSheet : data?.balanceSheet;
    let res = statements || [];
    if (input.limit) res = res.slice(0, input.limit);
    return formatToolResult(
      stripFieldsDeep(res, REDUNDANT_FINANCIAL_FIELDS),
...
export const getCashFlowStatements = new DynamicStructuredTool({
  ...
  func: async (input) => {
    const ticker = input.ticker.trim().toUpperCase();
    const { data, url } = await stocklens.getFinancialStatements(ticker);
    const statements = input.period === 'quarterly' ? data?.quarterly?.cashFlow : data?.cashFlow;
    let res = statements || [];
    if (input.limit) res = res.slice(0, input.limit);
    return formatToolResult(
      stripFieldsDeep(res, REDUNDANT_FINANCIAL_FIELDS),
...
export const getAllFinancialStatements = new DynamicStructuredTool({
  ...
  func: async (input) => {
    ...
    let inc = input.period === 'quarterly' ? data?.quarterly?.incomeStatement : data?.incomeStatement;
    let bal = input.period === 'quarterly' ? data?.quarterly?.balanceSheet : data?.balanceSheet;
    let cf = input.period === 'quarterly' ? data?.quarterly?.cashFlow : data?.cashFlow;
    
    if (input.limit) {
      inc = (inc || []).slice(0, input.limit);
      bal = (bal || []).slice(0, input.limit);
      cf = (cf || []).slice(0, input.limit);
    }
