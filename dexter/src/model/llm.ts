...
export class GeminiKeyRotator {
  ...
  getNextKey(): string {
    ...
    if (this.keys.length === 0) {
      try {
        ...
        throw new Error('[LLM] No Gemini API keys configured. Please set GEMINI_API_KEYS or GOOGLE_API_KEY in your .env file.');
      }
    }
  ...
  markCurrentKeyFailed() {
    if (this.keys.length > 1) {
      const failedKey = this.keys[this.currentIndex === 0 ? this.keys.length - 1 : this.currentIndex - 1];
      logger.warn(`[GeminiKeyRotator] Key ${failedKey.slice(0, 8)}... failed, rotating to next key.`);
    }
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      if (provider.toLowerCase() === 'google' || provider.toLowerCase() === 'gemini') {
        geminiKeyRotator.markCurrentKeyFailed();
      }
...
const MODEL_FACTORIES: Record<string, ModelFactory> = {
  ...
  google: (name, opts) =>
    new ChatGoogleGenerativeAI({
      model: name,
      ...opts,
      apiKey: geminiKeyRotator.getNextKey(),
    }),
  xai: (name, opts) =>
    new ChatOpenAI({
      model: name,
...
const DEFAULT_FACTORY: ModelFactory = (name, opts) =>
  new ChatOpenAI({
    ...
    ...(process.env.OPENAI_API_BASE ? { configuration: { baseURL: process.env.OPENAI_API_BASE } } : {}),
  });
...
export async function callLlm(prompt: string, options: CallLlmOptions = {}): Promise<LlmResult> {
  ...
  const result = await withRetry(executeCall, provider.displayName, maxAttempts);
...
export async function* streamLlmWithMessages(
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      attempt++;
      if (attempt >= maxAttempts || isNonRetryableError(message)) {
        throw new Error(`[${provider.displayName} API] ${message}`);
      }
