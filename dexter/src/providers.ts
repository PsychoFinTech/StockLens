...
export interface ProviderDef {
  ...
  displayName: string;
  /** Model name prefix used for routing. Can be string or array of strings. */
...
export const PROVIDERS: ProviderDef[] = [
  ...
  {
    id: 'google-gemma', // Keep as compatibility but route through main google provider mostly
    displayName: 'Gemma',
    modelPrefix: 'gemma-',
    apiKeyEnvVar: 'GOOGLE_API_KEY',
    fastModel: 'gemma-4-31b-it',
...
export function resolveProvider(modelName: string): ProviderDef {
  return (
    PROVIDERS.find((p) => {
      ...
      const prefixes = Array.isArray(p.modelPrefix) ? p.modelPrefix : [p.modelPrefix];
